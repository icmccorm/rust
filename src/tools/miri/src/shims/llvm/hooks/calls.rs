...
use crate::InterpError::UndefinedBehavior;
use crate::UndefinedBehaviorInfo::ScalarSizeMismatch;
...
fn miri_call_by_name_result<'tcx>(
    ...
    let (mut function_args, return_type_opt) = ctx.resolve_llvm_interface_unchecked(tref, args_ref);
    ...
    let gv_to_return = match name_rust_str {
        ...
        "strncmp" =>
            if num_args != 3 {
                ...
                let result = ctx.strcmp(&op_ty_args[0], &op_ty_args[1], Some(&op_ty_args[2]))?;
                GenericValue::from_byte_slice(&result.to_ne_bytes())
            },
        "srand" =>
            if num_args != 1 {
        ...
        _ => {
            ...
            match emulation_result {
                ...
                Ok(None) => {
                    let result_gv = ctx.perform_opty_conversion(&rplace, return_type_opt)?;
                    callback_places.push(rplace);
                    result_gv
                },
                Err(e) => {
                    if let UndefinedBehavior(ScalarSizeMismatch { .. }) = e.kind() {
                        if let Some(ref logger) = &ctx.machine.llvm_logger {
                            logger.log_flag(LLVMFlag::SizeMismatchInShim)
                        }
                    }
                    return Err(e);
                }   
